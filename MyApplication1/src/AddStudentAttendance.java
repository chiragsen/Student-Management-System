...
/**
 * Servlet implementation class AddStudentAttendance
 */
public class AddStudentAttendance extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			PreparedStatement st=conn.prepareStatement("insert into st_attendance values(?,?,?,?)");
			st.setString(1, id);
			...
			conn.commit();
			conn.close();
			request.getRequestDispatcher("admin_studentattendance.jsp").include(request,response);
		}
		catch(Exception e)
		{
